...
package com.blk.uhabits.helpers;
...
public class ListHabitsHelper
{
    ...
    public void updateFrequencyText(Habit habit,TextView tvFreq){
        int isChecked[] = habit.checkmarks.getAllValues();
        int count = 0;
        for (int i = 0; i < 7; i++) {
            if(isChecked.length > i){
        ...
        tvFreq.setText(count + "/7 recently");
    }

    public void updateAction(Habit habit, TextView tvActionArchieved, TextView tvActionEdit){
        tvActionArchieved.setTextColor(habit.color);
        tvActionEdit.setTextColor(habit.color);
        if(habit.isArchived()){
            tvActionArchieved.setText("Unarchive");
        } else {
            tvActionArchieved.setText("Archive");
        }
    }
    ...
    public void updateNameAndIcon(Habit habit, TextView tvStar, TextView tvName, TextView tvDescription)
    {
        int activeColor = getActiveColor(habit);
